...
public class PteranodonsBehaviour implements Behaviour {

	protected Location location;
	public PteranodonsBehaviour(Location location) {
	 ...
	 * and save the location and search for the food in that range and help Dinosaur to find foods. This Dinosaur can fly so if they 
	 * find any food in 2 locations further, they can fly instantly and get teh food
	 */
	@Override
				...
				if(xa<=map.getXRange().max() && ya<=map.getYRange().max() &&xa>=map.getXRange().min() &&ya>=map.getYRange().min()) 
				{
		            ...
		            if(groundItems.size() > 0){
			            for(int i = 0; i< groundItems.size();i++){
				            if(groundItems.get(i).getClass() == CorpseItem.class || groundItems.get(i).getClass() == Fish.class || groundItems.get(i).getClass() == CarnivoreItem.class){
