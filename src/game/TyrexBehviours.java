...
public class TyrexBehviours implements Behaviour {

	protected Location location;
	public TyrexBehviours(Location location) {
				...
				if(xa<=map.getXRange().max() && ya<=map.getYRange().max() &&xa>=map.getXRange().min() &&ya>=map.getYRange().min()) 
				{
		            ...
		            if(groundItems.size() > 0){
			            for(int i = 0; i< groundItems.size();i++){
				            if(groundItems.get(i).getClass() == CorpseItem.class || groundItems.get(i).getClass() == ProtoceratopsEggItem.class || groundItems.get(i).getClass() == VelociraptorsEggItem.class ||groundItems.get(i).getClass() == CarnivoreItem.class){
